...
async function createOrder(req, res) {
  try {
    ...
    console.log('[orders] Creating order, MongoDB connected:', mongoose.connection.readyState === 1);
    ...
    console.log('[orders] Mongoose connection state:', mongoose.connection.readyState);
...
async function listOrders(req, res) {
  try {
    ...
    // If we have a valid Order model and MongoDB connection, use it
    if (OrderModel && mongoose.connection.readyState === 1) {
      try {
    ...
    if (SHEETDB_URL) {
      try {
        ...
        if (r.ok) {
          ...
          console.log('[orders] Fetched from SHEETDB:', Array.isArray(list) ? list.length : 0, 'orders');
    ...
    console.log('[orders] No orders available - returning empty array');
    return res.json([]);
  } catch (e) {
  ...
  try {
    ...
    if (OrderModel && mongoose.connection.readyState === 1) {
      try {
        ...
        for (let d = new Date(start30); d <= now; d.setDate(d.getDate() + 1)) {
          ...
          daily.push({ date: key, orders: v ? v.orders : 0, revenue: v ? v.revenue : 0 });
        }
        const monthly = [];
        for (let m = new Date(start12m); m <= now; m.setMonth(m.getMonth() + 1)) {
          const key = `${m.getFullYear()}-${String(m.getMonth() + 1).padStart(2, '0')}`;
          const v = monthlyMap.get(key);
          monthly.push({ month: key, orders: v ? v.orders : 0, revenue: v ? v.revenue : 0 });
        }

        return res.json({ daily, monthly });
      } catch (dbErr) {
        console.error('[orders] report db error:', dbErr && dbErr.message ? dbErr.message : dbErr);
      }
    }
    ...
    if (SHEETDB_URL) {
      try {
        ...
        console.error('[orders] report sheetdb error:', sheetErr && sheetErr.message ? sheetErr.message : sheetErr);
      }
    }
...
module.exports = { createOrder, listOrders, getOrder, checkTransaction, myOrders, updateOrder, deleteOrder, reportOrders };
