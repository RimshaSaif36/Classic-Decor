...
async function getReviewerIdentity(userId) {
  ...
  if (UserModel && mongoose.connection.readyState === 1) {
    ...
    if (normalizedUserId.match(/^[0-9a-fA-F]{24}$/)) {
      user = await UserModel.findById(normalizedUserId).select('email name').lean();
    } else if (!isNaN(Number(normalizedUserId))) {
      user = await UserModel.findOne({ legacyId: Number(normalizedUserId) }).select('email name').lean();
    }
  ...
  if (email) {
    orConditions.push({ email: new RegExp(`^${email.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i') });
  }
...
async function createReview(req, res) {
  try {
    ...
    const orders = await OrderModel.find({ $or: orConditions }).select('items').lean();
    const hasPurchasedProduct = orders.some((order) => orderContainsProduct(order, productId));
    ...
    if (ReviewModel && mongoose.connection.readyState === 1) {
      const existingReview = await ReviewModel.findOne(existingReviewQuery).select('_id').lean();
      if (existingReview) {
      ...
      const hasExistingReview = list.some((review) => {
        ...
        if (String(review.userId || '').trim() === String(req.user.id || '').trim()) return true;
        if (legacyUserId !== null && Number(review.legacyUserId) === legacyUserId) return true;
        if (reviewerEmail && String(review.reviewerEmail || '').trim().toLowerCase() === reviewerEmail) return true;
    ...
    if (ReviewModel && mongoose.connection.readyState === 1) {
      ...
      const r = new ReviewModel({
        ...
        comment,
        name,
        userId: String(req.user.id).match(/^[0-9a-fA-F]{24}$/) ? req.user.id : null,
        legacyUserId,
        reviewerEmail,
        status,
        createdAt: now,
