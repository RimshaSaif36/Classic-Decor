...
const { computeShipping } = (() => {
  try {
    return require("../../utils/shipping");
  } catch (e) {
    try {
      return require("../../../backend/utils/shipping");
    } catch (e2) {
      return { computeShipping: (s) => (Number(s) > 5000 ? 0 : 200) };
    }
  }
})();
  ...
  try {
    ...
    const order = {
      ...
      items: cart
        .map(
          (i) =>
            `${i.name} x ${i.quantity} (PKR ${i.price})${i.size ? " | size: " + i.size : ""}${i.color ? " | color: " + i.color : ""}`,
        )
        .join(", "),
      sizes: cart
        .map((i) => i.size || "")
      ...
      if (order) {
        try {
          ...
          if (!r.ok) {
            ...
            console.log(
              "[stripe-success] posted order to SHEETDB successfully",
            );
          }
        } catch (err) {
          console.error(
            "[stripe-success] failed posting order to SHEETDB:",
