  ...
  async function handleSaveAnnouncement(e) {
    ...
    try {
      ...
      if (data && data.announcement) {
        ...
        window.dispatchEvent(new CustomEvent('announcement-updated', { detail: data.announcement }));
      }
      setAnnouncementMsg({ type: 'success', text: 'Announcement bar settings updated successfully!' });
      setTimeout(() => setAnnouncementMsg({ type: '', text: '' }), 4000);
    } catch (err) {
      setAnnouncementMsg({ type: 'error', text: err.message || 'Failed to update announcement settings.' });
    } finally {
            ...
            {restricted ? (
              ...
              <>
                ...
                {activeTab === 'announcement' && (
                  <div className="admin-section announcement-admin-panel" style={{ background: '#fff', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
                      <div>
                        <h3 className="admin-title" style={{ margin: 0 }}>Header Announcement Bar</h3>
                        <p style={{ margin: '6px 0 0', color: '#64748b', fontSize: '0.92rem' }}>
                          Manage the top announcement bar visible across the website header.
                        </p>
                      </div>
                      <span
                        style={{
                          ...
                          alignItems: 'center',
                          gap: '6px',
                          background: announcement.enabled ? '#e8f5e9' : '#ffebee',
                          color: announcement.enabled ? '#2e7d32' : '#c62828',
                          border: `1px solid ${announcement.enabled ? '#c8e6c9' : '#ffcdd2'}`,
                        }}
                      >
                        <i className={`fa-solid ${announcement.enabled ? 'fa-circle-check' : 'fa-circle-xmark'}`}></i>
                    ...
                    {announcementMsg.text && (
                      <div
                        style={{
                          ...
                          alignItems: 'center',
                          gap: '8px',
                          background: announcementMsg.type === 'success' ? '#e8f5e9' : '#ffebee',
                          color: announcementMsg.type === 'success' ? '#2e7d32' : '#c62828',
                          border: `1px solid ${announcementMsg.type === 'success' ? '#a5d6a7' : '#ef9a9a'}`,
                        }}
                      >
                        <i className={`fa-solid ${announcementMsg.type === 'success' ? 'fa-circle-check' : 'fa-triangle-exclamation'}`}></i>
                        {announcementMsg.text}
                      </div>
                    )}
                    ...
                    <div style={{ marginBottom: '25px' }}>
                      ...
                      >
                        {announcement.enabled && announcement.text ? (
                          <div
                            style={{
                              backgroundColor: announcement.bgColor || '#1a1a1a',
                          ...
                          >
                            ...
                            {announcement.link && (
                              <i className="fa-solid fa-arrow-right-long" style={{ fontSize: '0.8rem' }}></i>
                            )}
                          </div>
                        ) : (
                          <div
                          ...
                          >
                            {announcement.enabled
                              ? 'Announcement bar is enabled, but text is empty.'
                              : 'Announcement bar is currently disabled and hidden on the website.'}
                          </div>
                        )}
                      </div>
                    </div>

                    <form onSubmit={handleSaveAnnouncement} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                      ...
                      >
                        <input
                          type="checkbox"
                          id="announcement-enabled-check"
                          checked={announcement.enabled}
                          onChange={(e) => setAnnouncement((prev) => ({ ...prev, enabled: e.target.checked }))}
                          onClick={(e) => e.stopPropagation()}
                          style={{ width: '18px', height: '18px', cursor: 'pointer', accentColor: 'var(--primary, #0f172a)' }}
                        />
                        <label htmlFor="announcement-enabled-check" style={{ cursor: 'pointer', margin: 0, fontWeight: '600', fontSize: '0.95rem' }} onClick={(e) => e.stopPropagation()}>
                          Enable Announcement Bar on top of Header
                        </label>
                      </div>
                      ...
                      <div>
                        <label style={{ display: 'block', fontWeight: '600', marginBottom: '6px', color: '#334155' }}>
                          Announcement Text <span style={{ color: '#e53935' }}>*</span>
                        </label>
                        <textarea
                          rows={2}
                          placeholder="e.g. 🎉 Free Delivery Nationwide on orders over PKR 3,000! Limited Time Offer"
                        ...
                        <span style={{ fontSize: '0.8rem', color: '#64748b' }}>
                          This message will be shown across all pages at the top of the header.
                        </span>
                      </div>
                      ...
                      <div>
                        <label style={{ display: 'block', fontWeight: '600', marginBottom: '6px', color: '#334155' }}>
                          Clickable Link / URL (Optional)
                        </label>
                        <input
                          type="text"
                          placeholder="e.g. /shop or /categories/wall-art or https://..."
                      ...
                      {/* QUICK COLOR PRESETS */}
                      <div>
                        <label style={{ display: 'block', fontWeight: '600', marginBottom: '8px', color: '#475569', fontSize: '0.85rem' }}>
                          Quick Color Themes:
                        </label>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                          {[
                            { name: 'Dark Luxe', bg: '#1a1a1a', text: '#ffffff' },
                            { name: 'Classic Gold', bg: '#854d0e', text: '#ffffff' },
                            { name: 'Warm Amber', bg: '#b45309', text: '#ffffff' },
                            { name: 'Deep Navy', bg: '#0f172a', text: '#38bdf8' },
                            ...
                            <button
                              key={theme.name}
                              type="button"
                              onClick={() => setAnnouncement((prev) => ({ ...prev, bgColor: theme.bg, textColor: theme.text }))}
                              style={{
                                display: 'inline-flex',
                                alignItems: 'center',
                                gap: '6px',
                            ...
                            >
                              <span style={{ width: '12px', height: '12px', borderRadius: '50%', background: theme.bg, border: '1px solid #94a3b8' }}></span>
                              {theme.name}
                            </button>
                          ))}
                        </div>
                      ...
                      <div style={{ marginTop: '10px' }}>
                        ...
                        >
                          <i className={`fa-solid ${announcementSaving ? 'fa-spinner fa-spin' : 'fa-floppy-disk'}`}></i>
                          {announcementSaving ? 'Saving Changes...' : 'Save Announcement Settings'}
                        </button>
                      </div>
                    </form>
                  </div>
